<div class="flex flex-col gap-3">
    <x-main.page-header title="Pengeluaran Proyek">
        ...
        <div class="relative">
            <div
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-base-content/60 mt-1 flex items-center gap-1">
                            <x-lucide-map-pin class="size-3" /> {{ $proyek->lokasi ?? '-' }} • <x-lucide-user class="size-3" />
                            {{ $proyek->pemilik ?? '-' }}
                        </p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('proyek.index') }}" wire:navigate class="btn btn-sm btn-ghost">
                        <x-lucide-arrow-left class="size-4" /> Kembali
                    </a>
                </div>
            </div>

            <div class="grid grid-cols-6 gap-4 px-5 py-4 text-sm">
                <div class="col-span-6 md:col-span-3 lg:col-span-2">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Kode Proyek</span>
                    <span class="font-mono font-semibold">{{ $proyek->kode_proyek ?? '-' }}</span>
                </div>
                <div class="col-span-6 md:col-span-3 lg:col-span-4">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Nama Proyek</span>
                    <span class="font-semibold">{{ $proyek->nama_proyek ?? '-' }}</span>
                </div>
                <div class="col-span-6 md:col-span-3">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Pemilik</span>
                    <span class="font-semibold flex items-center gap-1"><x-lucide-user class="size-3 text-base-content/50" />
                        {{ $proyek->pemilik ?? '-' }}</span>
                </div>
                <div class="col-span-6 md:col-span-3">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Lokasi</span>
                    <span class="font-semibold flex items-center gap-1"><x-lucide-map-pin class="size-3 text-base-content/50" />
                        {{ $proyek->lokasi ?? '-' }}</span>
                </div>
                <div class="col-span-6 md:col-span-3">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Periode Proyek</span>
                    <span class="font-semibold flex items-center gap-1"><x-lucide-calendar-range class="size-3 text-base-content/50" />
                        {{ $proyek->tanggal_mulai?->format('d/m/Y') ?? '-' }} — {{ $proyek->tanggal_selesai?->format('d/m/Y') ?? '-' }}</span>
                </div>
                <div class="col-span-6 md:col-span-3">
                    <span class="block text-[10px] font-semibold tracking-widest uppercase text-base-content/50">Dibuat</span>
                    <span class="font-semibold flex items-center gap-1"><x-lucide-clock class="size-3 text-base-content/50" />
                        {{ $proyek->created_at?->format('d/m/Y H:i') ?? '-' }}</span>
                </div>
            </div>
        </div>
    </div>

    {{-- Helper: quick jump antar modul proyek --}}
    <div class="flex flex-wrap gap-2" data-motion="fade-up">
        <a href="{{ route('proyek.pekerja.index', $proyek->id) }}" wire:navigate class="btn btn-sm btn-outline gap-1">
            <x-lucide-users class="size-4" /> Pekerja
        </a>
        <span class="btn btn-sm btn-primary gap-1 pointer-events-none">
            <x-lucide-receipt class="size-4" /> Pengeluaran
        </span>
        <a href="{{ route('penggajian.index', ['filterProyekId' => $proyek->id]) }}" wire:navigate class="btn btn-sm btn-outline gap-1">
            <x-lucide-wallet class="size-4" /> Penggajian
        </a>
        <a href="{{ route('proyek.index') }}" wire:navigate class="btn btn-sm btn-ghost gap-1">
            <x-lucide-arrow-left class="size-4" /> Daftar Proyek
        </a>
    </div>

    {{-- Stats Overview --}}
    <div class="grid grid-cols-12 gap-3" data-motion="stagger">
        <div class="col-span-12 md:col-span-6 lg:col-span-4">
            <div class="stats stats-vertical lg:stats-horizontal shadow border border-base-300 bg-base-100 w-full" data-motion="pop-in"
                data-motion-delay="0.05">
                <div class="stat">
                    <div class="stat-figure text-primary"><x-lucide-receipt class="size-6" /></div>
                    <div class="stat-title text-xs">Total Pengeluaran</div>
                    <div class="stat-value text-2xl">{{ $stats['total'] ?? 0 }}</div>
                    <div class="stat-desc">entri tercatat</div>
                </div>
            </div>
        </div>
        <div class="col-span-12 md:col-span-6 lg:col-span-4">
            <div class="stats stats-vertical lg:stats-horizontal shadow border border-base-300 bg-base-100 w-full" data-motion="pop-in"
                ...
                <div class="stat">
                    <div class="stat-figure text-success"><x-lucide-wallet class="size-6" /></div>
                    <div class="stat-title text-xs">Total Nominal</div>
                    <div class="stat-value text-lg">Rp {{ number_format($stats['sumNominal'] ?? 0, 0, ',', '.') }}</div>
        ...
        <div class="col-span-12 md:col-span-12 lg:col-span-4">
            <div class="stats stats-vertical lg:stats-horizontal shadow border border-base-300 bg-base-100 w-full" data-motion="pop-in"
                ...
                <div class="stat">
                    <div class="stat-figure text-warning"><x-lucide-tags class="size-6" /></div>
                    <div class="stat-title text-xs">Kategori Top</div>
                    <div class="stat-value text-sm">
                        @if (($stats['kategoriTop'] ?? collect())->isNotEmpty())
                            {{ $stats['kategoriTop']->keys()->first() }}
                        @else
                            -
                        @endif
                    </div>
                    <div class="stat-desc">
                        @if (($stats['kategoriTop'] ?? collect())->isNotEmpty())
                            @foreach ($stats['kategoriTop'] as $kat => $cnt)
                                <span class="badge badge-xs badge-ghost">{{ $kat }}: {{ $cnt }}</span>
                            @endforeach
                        @else
                            belum ada data
                        @endif
    ...
    <div class="grid grid-cols-12 gap-3">
        <div class="col-span-12 lg:col-span-8">
            <div class="card border border-base-300 bg-base-100" data-motion="fade-up" data-motion-delay="0.1">
                <div class="card-body p-4">
                    <h4 class="font-semibold text-sm flex items-center gap-2">
                        <x-lucide-lightbulb class="size-4 text-warning" /> Tips Pencatatan
                    </h4>
                    <p class="text-xs text-base-content/60 mt-2">Catat setiap pembelian bahan, semen, peralatan dengan tanggal & nominal Rupiah (auto-format). Kategori membantu rekap per jenis pengeluaran.</p>
                </div>
            </div>
        </div>
        <div class="col-span-12 lg:col-span-4">
            <div class="card border border-base-300 bg-base-100" data-motion="fade-up" data-motion-delay="0.15">
                <div class="card-body p-4">
                    <h4 class="font-semibold text-sm flex items-center gap-2">
                        <x-lucide-sparkles class="size-4 text-warning" /> Aksi Cepat
                    </h4>
                    <div class="grid grid-cols-2 gap-2 mt-3">
                        <button type="button" class="btn btn-sm btn-primary" wire:click="showForm(true)" @if ($form) disabled @endif>
                            <x-lucide-plus class="size-4" /> Tambah
                        </button>
                        <a href="{{ route('proyek.pekerja.index', $proyek->id) }}" wire:navigate class="btn btn-sm btn-outline gap-1">
                            <x-lucide-users class="size-4" /> Pekerja
                        </a>
                        <a href="{{ route('penggajian.index', ['filterProyekId' => $proyek->id]) }}" wire:navigate class="btn btn-sm btn-outline gap-1 col-span-2">
                            <x-lucide-wallet class="size-4" /> Penggajian
                        </a>
                    </div>
                </div>
